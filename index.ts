...
import {QR_API_DEFAULT_PORT} from './constants'
...
(async () => {
    ...
    info('系统初始化完毕')
    startQrApi(QR_API_DEFAULT_PORT)
    info('')
            ...
            try {
                ...
                if (checkinInfo.type === 'qr') {
                    ...
                    const results = await Promise.all(config.accounts.map(async (account) => {
                        ...
                        return `${am.name}：${ret === 'success' ? '✅ 成功' : ret}`
                    }))
    ...
    fs.watch(qrDir, async (eventType, filename) => {
        ...
        // 清理 Set 防止内存泄漏（超过 100 条时清空已处理记录）
